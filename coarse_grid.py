...
class Params(object):
    ...
    def __init__(self, basedir, sparse_param_names=None, sparse_param_limits=None):
        if sparse_param_names is None:
            self.sparse_param_names = ['ns', 'As', 'heat_slope', 'heat_amp', 'hub']
        else:
            self.sparse_param_names = sparse_param_names
        if sparse_param_limits is None:
            self.sparse_param_limits = np.array([[0.6, 1.5], [1.5e-9, 4.0e-9], [0., 0.5],[0.25,2],[0.65,0.75]])
        else:
            self.sparse_param_limits = sparse_param_limits
    ...
    def build_dirname(self,params):
        ...
        assert len(params) == len(self.sparse_param_names)
        name = ""
        for nn,pp in zip(self.sparse_param_names, params):
            name += nn+'%.2g' % pp
        return name
    ...
    def dump(self, dumpfile="emulator_params.json"):
        ...
        self.sparse_param_limits = self.sparse_param_limits.tolist()
        ...
        self.sparse_param_limits = np.array(self.sparse_param_limits)
    ...
    def build_params(self, nsamples):
        ...
        self.sample_params = latin_hypercube.get_hypercube_samples(self.sparse_param_limits, nsamples)
    ...
    def gen_simulations(self, nsamples, npart=256.,box=60,):
        ...
        for ev,edir in zip(self.sample_params, self.sample_dirs):
            ...
            assert self.sparse_param_names[0] == 'ns'
            assert self.sparse_param_names[1] == 'As'
            assert self.sparse_param_names[2] == 'heat_slope'
            assert self.sparse_param_names[3] == 'heat_amp'
            assert self.sparse_param_names[4] == 'hub'
