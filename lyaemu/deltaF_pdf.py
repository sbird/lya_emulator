...
class FluxPdf(object):
    ...
    def get_deltaF_pdf(self, mean_fluxes, bins=np.arange(-1.0, 1.0, 0.02)):
        ...
        deltaF_arr = np.empty(shape=(self.len(),np.size(mbin)))
        for (i,ss) in enumerate(self.spectrae):
            if mean_fluxes is not None:
                mf = mean_fluxes[i]
                tau = ss.get_tau(elem='H', ion=1, line=1215)
                flux = np.exp(-1.0*tau)
                flux = flux - np.average(flux) + mf
                deltaF = (flux/mf) - 1.0
            deltaF_array[i] = np.histogram(deltaF, bins=bins, density=True)[0]
            ...
            #flux_arr[i] = ff
        deltaF_array = np.ravel(deltaF_array)
        assert np.shape(deltaF_arr) == (self.len()*np.size(mbin),)
        self.drop_table()
        return delaF_arr
...
class MySpectra(object):
    ...
    def __init__(self, numlos = 32000, max_z= 4.2, bins = np.arange(-1.0, 1.0, 0.02)):
        self.NumLos = numlos
