...
#
#             gp = GPy.core.GP(params, normfps[:, [i]], kernel, likelihood, Y_metadata=y_metadata)
...
#             # fix noise
#             getattr(gp.mixed_noise, "Gaussian_noise").fix(1e-1)
#             for j in range(1, self.n_fidelities):
#                 getattr(gp.mixed_noise, "Gaussian_noise_{}".format(j)).fix(1e-1)
...
#             # this is not ideal -- the GPy package may have better methods in future updates
#             try:
#                 zbin_file = os.path.join(os.path.abspath(self.traindir), 'zbin'+str(self.zbin)+'/kbin'+str(i+1))
#                 trained_model = self.load_GP(model, zbin_file)
#                 print('Loading pre-trained GP for z:'+str(self.zbin)+'/kbin'+str(i+1))
#                 models.append(trained_model)
#             except:
#                 print('Optimizing GP for z:'+str(self.zbin)+'/kbin'+str(i+1))
...
#                 model.gpy_model.optimize_restarts(n_optimization_restarts//2, verbose=model.verbose_optimization, robust=True, parallel=False,)
#                 models.append(model)
#                 # save trained GP, if not already
#                 # this is not ideal -- the GPy package may have better methods in future updates
#                 if self.traindir != None: # if a traindir was requested, but not populated
#                     print('Saving GP to', zbin_file)
#                     gpdir = os.path.join(os.path.abspath(self.traindir), 'zbin'+str(self.zbin))
#                     if not os.path.exists(gpdir): os.makedirs(gpdir)
...
#
#         fps_predict = np.zeros(self.scalefactors.shape)
#         var = np.zeros(self.scalefactors.shape)
#         for i, model in enumerate(self.models):
#             fps_predict[i], var[i] = model.predict(params_cube)
#
