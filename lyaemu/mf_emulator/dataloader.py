...
def folder_name(num1: int, res1: int, box1: int, num2: int, res2:int, box2: int, z: float):
    return "Lyaflux_{}_res{}box{}_{}_res{}box{}_{}".format(
        num1, res1, box1, num2, res2, box2, str(z).replace(".", "_")
    )
...
def convert_h5_to_txt(
    ...
    print("Shape of flux vectors\n", f_lf["flux_vectors"].shape)

    # use kfmpc so all redshifts use the same k bins
    kfmpc = f_lf["kfmpc"][()]
    assert np.all(kfmpc == f_hf["kfmpc"][()])
    ...
    for i,z in enumerate(zout):
        ...
        np.savetxt(os.path.join("data", outdir, "train_output_fidelity_0.txt"), flux_vector_lf)
        np.savetxt(os.path.join("data", outdir, "train_output_fidelity_1.txt"), flux_vector_hf)

        np.savetxt(os.path.join("data", outdir, "train_input_fidelity_0.txt"), x_train_lf)
        np.savetxt(os.path.join("data", outdir, "train_input_fidelity_1.txt"), x_train_hf)
