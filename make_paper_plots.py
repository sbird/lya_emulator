...
def test_knot_plots(mf=1, testdir = None, emudir = None, plotname="", kf_bin_nums=None):
    """Plot emulator test-cases"""
    if testdir is None:
        testdir = path.expanduser("~/data/Lya_Boss/hires_knots_test")
    if emudir is None:
        emudir = path.expanduser("~/data/Lya_Boss/hires_knots")
    gp_emu = coarse_grid_plot.plot_test_interpolate(emudir, testdir,savedir=path.join(plotdir,"hires_knots_mf"+str(mf)),plotname=plotname,mean_flux=mf,kf_bin_nums=kf_bin_nums)
    return gp_emu
