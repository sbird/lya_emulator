...
ff = h5py.File(basedir+'hires/loo_fps_3.hdf5', 'r')
...
ax[0].plot(bins, 300 * np.exp(-0.5*4*bins**2), color="black")
ax[0].set_yscale('log')

logbins = np.logspace(np.log10(err.min()), np.log10(err.max()), 280)
ax[1].hist(err, bins=logbins, color=c_midnight, histtype='stepfilled', alpha=0.9, label='MF 2')
...
fig.savefig(figbase+"fpsemu_errors_2.pdf", bbox_inches='tight', pad_inches=0.075)
plt.show()

ff = h5py.File(basedir+'loo_fps_2.hdf5', 'r')
fpp, fpt, pp, std = ff['flux_predict'][:], ff['flux_true'][:], ff['params'][:], ff['std_predict'][:]
ff.close()
err = np.abs(fpp/fpt-1).flatten()
errSTD = ((fpp-fpt)/std).flatten()
...
fig.patch.set_facecolor('none')
fig.subplots_adjust(hspace=0, wspace=0)
fig.savefig(figbase+"fpsemu_errors_sf_2.pdf", bbox_inches='tight', pad_inches=0.075)
plt.show()
